# Train models for Experiment D (Big-Vul only) and Experiment E (Union)
# Each experiment trains 4 encoder models with 2 epochs, patience=1
...
Write-Host "`n===== EXPERIMENT D: Big-Vul Only =====" -ForegroundColor Cyan

foreach ($model in $models) {
    $variant = $model.Split("/")[-1]
    Write-Host "`n  Training $variant for Exp D..." -ForegroundColor Yellow

    python -m src.train `
        --experiment exp_d_bigvul `
        --model $model `
        --train-path data/processed/bigvul_train.parquet `
...
Write-Host "`n===== EXPERIMENT E: Juliet + Big-Vul Union =====" -ForegroundColor Cyan

foreach ($model in $models) {
    $variant = $model.Split("/")[-1]
    Write-Host "`n  Training $variant for Exp E..." -ForegroundColor Yellow
